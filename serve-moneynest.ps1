...
if (Test-Path (Join-Path $PSScriptRoot "moneynest-app.html") -PathType Leaf) {
  $root = $PSScriptRoot
} elseif (Test-Path (Join-Path $PSScriptRoot "MoneyNest\\moneynest-app.html") -PathType Leaf) {
  $root = Join-Path $PSScriptRoot "MoneyNest"
} else {
  throw "Could not find moneynest-app.html relative to $PSScriptRoot"
}
...
Write-Host "MoneyNest server running."
Write-Host "Serving from: $root"
Write-Host "This PC: http://localhost:$port/moneynest-app.html"
if ($localIp) {
  Write-Host "Mobile: http://$localIp`:$port/moneynest-app.html"
  ...
  while ($true) {
    ...
    try {
      ...
      if ([string]::IsNullOrWhiteSpace($requestPath)) {
        $requestPath = "moneynest-app.html"
      }
